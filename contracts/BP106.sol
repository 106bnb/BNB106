...
contract Pmm is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _recordReferral(address _user, address _referrer) internal {
        ...
        // invalid address
        if (
            _user == _referrer || _user == address(0) || _referrer == address(0)
            // _user.isContract() ||
    ...
    function directReward(address user) internal {
        address _referrer = referrerOfUser[user];
    ...
    function bossReward(address me) internal {
        ...
        if (identityOfUser[me] == 2) {
            boss1 = getUpBoss(me);
        } else if (identityOfUser[me] == 1) {
            boss1 = getUpBoss(getUpBoss(me));
        }
    ...
    function ti(address me) internal {
        if (identityOfUser[me] != 2 || me == firstAddress) {
            return;
        }
        ...
        for (uint8 i = 0; i < 2; i++) {
            address A = usersOfReferrer[me][i];
            uint256 Atimes = userJoinTime[A];
            if (
                (Atimes + hours168) < block.timestamp && identityOfUser[A] == 1
            ) {
                bossStatus[me] = 2;
                changeFinancialReferrer(A, me);
                uint256 len = countUsersOfReferrer(me);
                if (len > 0) {
                    address newA = usersOfReferrer[me][len - 1];
                    usersOfReferrer[me][i] = newA;
                    usersOfReferrer[me][len - 1] = A;

                    changeFinancialReferrer(newA, _referrer);
                }
                return;
            }
        }
    ...
    function _removeUsersOfReferrer(address referrer, address _downuser)
        private
    {
